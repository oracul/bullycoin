...
{
    QString ret = QString("bullyon:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BUL unit input in lnReqAmount, we generate the URI with BUL as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(bullyonUnits::format(bullyonUnits::BUL, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
